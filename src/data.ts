...
/**
 * Убирает "мусорные" префиксы, случайно попавшие в начало названия товара
 * при выгрузке из прайса/1С: служебный артикул (числовой или буквенно-
 * цифровой код), иногда с номером пункта списка перед ним
 * ("9. Y-355-350, Сверло..." -> "Сверло...").
 *
 * НЕ трогает:
 * - бренды в начале названия (BRITE, ELASTA) — это осознанно оставленная
 *   информация, а не мусор;
 * - маркеры материала "П/П"/"М/П" в начале (полипропилен/металлопласт) —
 *   то же самое, полезная информация;
 * различие делается по наличию цифры в самом коде: у артикулов она есть
 * (IMK00390, SQ0548-0607, 685200, P20313M, 304...), у брендов/маркеров
 * материала — нет.
 *
 * Также чинит баг выгрузки, когда всё название задвоено целиком
 * ("...M10743Анкерный болт...M10743" -> "...M10743").
 */
export function cleanProductName(rawName: string): string {
  let s = rawName;

  // Всё название продублировано подряд без разделителя.
  const half = s.length / 2;
  if (Number.isInteger(half) && half > 0 && s.slice(0, half) === s.slice(half)) {
    s = s.slice(0, half);
  }

  // "9. " — номер пункта списка перед артикулом.
  s = s.replace(/^\d{1,3}\.\s+/, "");

  // Служебный код + запятая + настоящее название с большой буквы:
  // "IMK00390, Саморез..." / "SQ0548-0607, Трубка..." / "685200, Лампа...".
  // Код — только цифры/заглавные буквы/дефисы/точки/слэши (без строчных букв,
  // иначе задело бы обычные слова вроде "Дюбель-гвоздь, потай...").
  s = s.replace(
    /^\.?[A-ZА-ЯЁ0-9][A-ZА-ЯЁ0-9\-./]{1,19},\s+(?=[A-ZА-ЯЁ][a-zа-яёA-ZА-ЯЁ])/,
    ""
  );

  // Тот же служебный код, но без запятой перед названием:
  // "P20313M Гайка..." / "304 Конвектор...". Срезаем только если в коде
  // есть цифра — этим отличаем от брендов/маркеров материала без цифр
  // (BRITE, ELASTA, П/П, М/П), которые НЕ мусор и должны остаться.
  s = s.replace(
    /^([A-ZА-ЯЁ0-9][A-ZА-ЯЁ0-9\-./]{1,19})\s+(?=[А-ЯЁ][а-яё])/,
    (m, code: string) => (/\d/.test(code) ? "" : m)
  );
...
export function getPriceItems(): PriceItem[] {
  ...
  return products.map((item, index) => {
    const name = cleanProductName(item.name) || `Товар #${index + 1}`;
...
export const SERVICES: Service[] = [
  ...
  },
  {
    ...
    description: "Монтаж внутренних систем электроснабжения зданий и сооружений.",
  ...
  },
  {
    ...
    description: "Монтаж и сервисное обслуживание систем водоснабжения и отопления.",
