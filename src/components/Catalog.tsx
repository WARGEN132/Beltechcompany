...
export default function Catalog({
  ...
  // Ключ для запоминания позиции скролла и "показано N" по конкретному
  // списку (подкатегория + поисковый запрос) — чтобы после возврата со
  // страницы товара попасть туда же, где был клик, а не в начало каталога.
  const scrollKey = `catalog-scroll:${subcategorySlug || "all"}:${searchQuery}`;

  // Пагинация внутри списка товаров — крупные подкатегории (50-200+ позиций)
  // рендерятся не все сразу, а порциями, с кнопкой "Показать ещё". Начальное
  // значение читаем из sessionStorage — если человек до этого пролистал до
  // 96-й позиции и открыл товар, при возврате должны сразу отрисоваться все
  // 96, иначе прокрутка к сохранённому scrollY попадёт в пустоту.
  const PAGE_SIZE = 24;
  const [visibleCount, setVisibleCount] = useState(() => {
    try {
      const saved = sessionStorage.getItem(scrollKey);
      if (saved) return Math.max(PAGE_SIZE, JSON.parse(saved).visibleCount || PAGE_SIZE);
    } catch {}
    return PAGE_SIZE;
  });
  ...
  // Переходы между "разделами" / "категориями" / "подкатегориями" внутри каталога
  // (клик по разделу, по категории, кнопки "назад"/"все категории") меняют ТОЛЬКО
  // query-параметры (?section=...&category=...), а не сам URL-путь — поэтому
  // глобальный сброс скролла в App.tsx (он срабатывает на смену pathname) тут
  // не помогает: страница остаётся там, где её пролистали, хотя контент под
  // ней резко становится короче (например, карточки категорий вместо длинного
  // списка товаров) — из-за этого и кажется, что после клика "улетает вниз".
  // Для списка товаров (viewMode === "products") этот сброс НЕ делаем — там
  // работает отдельная, намеренная логика восстановления прежней позиции
  // скролла при возврате со страницы товара (см. useLayoutEffect ниже).
  React.useEffect(() => {
    if (viewMode !== "products") {
      window.scrollTo(0, 0);
    }
  }, [viewMode, activeSection, activeCategoryName]);
  ...
  const SECTIONS: { id: "santehnika" | "electrika"; title: string; description: string; icon: React.ComponentType<{ className?: string }>; image?: string }[] = [
    {
      ...
      icon: Droplets,
      image: "/images/catalog/products/zadvizhka-30.jpg",
    },
    {
      id: "electrika",
      title: "Электрика",
  ...
  // от предыдущего (например, более длинного) списка. Сравниваем с ПРЕДЫДУЩИМ
  // значением ключа через ref, а не флагом "это первый рендер" — простой флаг
  // ломается под React StrictMode (в dev-режиме эффекты монтирования
  // намеренно запускаются дважды), из-за чего восстановленный список иногда
  // тут же сбрасывался обратно, а с ним резко "укорачивалась" страница и
  // браузер утягивал скролл вверх. Сравнение по значению ключа устойчиво к
  // двойному запуску — сработает, только если ключ РЕАЛЬНО изменился.
  const prevResetKeyRef = React.useRef<string | null>(null);
  React.useEffect(() => {
    const key = `${activeSubcategory?.id || ""}::${searchQuery}`;
    if (prevResetKeyRef.current !== null && prevResetKeyRef.current !== key) {
      setVisibleCount(PAGE_SIZE);
    }
    prevResetKeyRef.current = key;
  }, [activeSubcategory?.id, searchQuery]);

  // Восстанавливаем прокрутку при возврате на этот же список (например, кнопкой
  // "назад" со страницы товара) — ДО отрисовки кадра (useLayoutEffect, не
  // useEffect), иначе страница на мгновение показывается вверху и потом
  // дёргается вниз — именно это и выглядело как "странно листает".
  // visibleCount уже восстановлен синхронно (см. lazy-инициализатор выше),
  // поэтому нужная позиция физически существует в разметке уже к этому моменту.
  ...
  // Непрерывно запоминаем текущую позицию скролла и "показано N" для этого
  // списка — а на размонтировании (уход на страницу товара) сохраняем
  // финальное значение ещё раз, чтобы при возврате оказаться там же.
  React.useEffect(() => {
    let ticking = false;
    const save = () => {
      try {
        sessionStorage.setItem(scrollKey, JSON.stringify({ scrollY: window.scrollY, visibleCount }));
      } catch {}
      ticking = false;
    };
    const onScroll = () => {
  ...
  return (
    <section id="catalog" className="py-12 sm:py-20 bg-[#f6f6f4] min-h-screen">
      <Helmet>
        ...
        {/* Open Graph/Twitter — превью при шаринге ссылки на подкатегорию/поиск */}
